...
class Application(tk.Frame):
    ...
    def create_widget(self):
        ...
        self.mouse_position = tk.Label(
            self.statusbar, relief=tk.SUNKEN, text="mouse position")  # マウスの座標
        self.image_position = tk.Label(
            self.statusbar, relief=tk.SUNKEN, text="image position")  # 画像の座標
        self.label_space = tk.Label(
            self.statusbar, relief=tk.SUNKEN)                           # 隙間を埋めるだけ
        self.image_info = tk.Label(
            self.statusbar, relief=tk.SUNKEN, text="image info")         # 画像情報
    ...
    def mouse_move(self, event):
        ...
        # 逆行列（画像→Cancasの変換からCanvas→画像の変換へ）
